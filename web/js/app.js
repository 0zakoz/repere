...
function onSubmit(event){event.preventDefault();try{if(event.target.id==="nutrition-form"){const data=new FormData(event.target);const next=saveNutrition(state,{id:data.get("id")||null,date:selectedNutritionDate,calories:data.get("calories"),protein:data.get("protein")});editingNutritionId=null;persist(next);toast("Apport enregistré")}if(event.target.id==="library-form")saveLibraryForm(event.target)}catch(error){toast(error.message,"error")}}
