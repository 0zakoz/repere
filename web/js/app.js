...
import { loadAppearance, loadNutritionTargets, loadState, requestPersistentStorage, saveAppearance, saveNutritionTargets, saveState } from "./store.js";
import { uid } from "./state.js";
import {
  abandonDraft, activeProgram, backupSummary, chronology, completeWorkout, editCompletedWorkout, exerciseHistory,
  acknowledgeMissedSlots, formatDuration, isSetValid, lastPerformedExercise, missedSlotCount, muscleStats,
  nutritionTrend, normalizeTargetCalories, normalizeTargetProtein, nutritionRemaining, saveNutrition, saveWeight, sessionStats, shiftDate, skipMissedSlots, startWorkout,
...
import { button, catSvg, dialog, downloadFile, emptyState, formatDate, formatTime, html, icon, number, toast } from "./ui.js";
...
let nutritionTargets = loadNutritionTargets();
let timerTick = null;
let saveQueue = Promise.resolve();

function reportError(message) {
  if (!message || /^\s*$/.test(message) || /ResizeObserver loop|Script error/i.test(message)) return;
  ...
  banner.querySelector("span").textContent = `Erreur : ${message}`;
}

if (typeof window !== "undefined") {
  window.addEventListener("error", event => reportError(event.message || event.error?.message));
...
function renderNav() {
  const items = [["journal","🐰","Journal"],["weight","🐼","Poids"],["nutrition","🍓","Nutrition"],["trends","🐱","Tendances"],["library","🎀","Biblio"]];
  return `<nav class="nav" aria-label="Navigation principale">${items.map(([id, emoji, label]) => `<button data-action="tab" data-tab="${id}" class="${tab === id ? "active" : ""}"><span class="nav-icon">${appearance.theme === "kawaii" ? kawaiiFace(emoji) : navIcon(id)}</span><span>${label}</span></button>`).join("")}</nav>`;
}

const navIcon = id => ({ journal:"●", weight:"◆", nutrition:"♥", trends:"↗", library:"▤" })[id];
const kawaiiFace = emoji => emoji === "🐱" ? catSvg() : emoji;
const kawaii = emoji => appearance.theme === "kawaii" ? `<span class="mascot">${kawaiiFace(emoji)}</span><span class="kawaii-deco heart" aria-hidden="true">♥</span><span class="kawaii-deco star" aria-hidden="true">✦</span>` : "";
const mascotFor = index => ["🐰", "🐼", "🐱"][((index % 3) + 3) % 3];
const tintFor = index => { const i = ((index % 5) + 5) % 5; return i === 0 ? "" : `tint-${i}`; };
const tinted = index => appearance.theme === "kawaii" ? tintFor(index) : "";
const hashStr = value => [...String(value)].reduce((sum, ch) => sum + ch.codePointAt(0), 0);
const surfaceVarFor = index => ["--surface", "--surface-2", "--surface-3", "--surface-4", "--surface-5"][((index % 5) + 5) % 5];
const mascotBadge = (emoji, index) => appearance.theme === "kawaii" ? `<span class="mascot-badge" style="background:var(${surfaceVarFor(hashStr(emoji) + index)})" aria-hidden="true">${kawaiiFace(emoji)}</span>` : "";
...
function renderWorkout(workout) {
  ...
  return `<section class="screen workout"><header class="workout-header"><button class="icon-btn" data-action="leave-workout">${icon("back")}</button><h1>Séance ${html(workout.templateNameSnapshot)}</h1><button class="icon-btn" data-action="finish-workout" aria-label="Terminer">${icon("check")}</button></header><div class="form-grid"><label class="field"><span>Date</span><input type="date" max="${localDate()}" value="${workout.localDate}" data-workout-field="localDate"></label><label class="field"><span>Début</span><input value="${formatTime(workout.startedAt)}" disabled></label></div><button class="duration-edit" data-action="edit-workout-duration"><span>${icon("timer")} Durée</span><strong data-workout-elapsed data-start="${workout.startedAt}" data-end="${workout.endedAt ?? ""}">${formatClock((durationEnd-workout.startedAt)/1000,true)}</strong>${icon("edit")}</button><label class="field"><span>Note</span><textarea data-workout-field="note" placeholder="Ressenti, contexte…">${html(workout.note)}</textarea></label>${workout.exercises.map((exercise,i) => renderWorkoutExercise(workout,exercise,i)).join("")}<footer class="workout-footer">${button("Exercice","add-workout-exercise",{kind:"secondary",iconName:"add"})}${button("Terminer","finish-workout",{iconName:"check"})}</footer>${timer ? `<div class="timer-float" data-start="${timer.restStartedAt}"><div class="timer-copy">${icon("timer")}<span><strong data-timer>${elapsed(timer.restStartedAt)}</strong><small>avant ${html(timer.nameSnapshot)} · S${targetSet?.order ?? "—"}</small></span></div><div class="timer-actions"><button data-action="focus-next-set" data-exercise-id="${timer.id}" data-set-id="${targetSet?.id ?? ""}">Série suivante</button><button data-action="stop-timer" data-exercise-id="${timer.id}" aria-label="Ignorer le chrono">×</button></div></div>` : ""}</section>`;
}

function renderWorkoutExercise(workout, exercise, index) {
  return `<article class="card ${tinted(index)}" data-exercise="${exercise.id}"><div class="card-title">${mascotBadge(mascotFor(index), index)}<div><h2>${html(exercise.nameSnapshot)}</h2><small class="range-hint">${exercise.repMinSnapshot}–${exercise.repMaxSnapshot} reps</small></div><div class="row-actions"><button class="icon-btn" data-action="move-workout-exercise" data-exercise-id="${exercise.id}" data-delta="-1" aria-label="Monter" ${index===0?"disabled":""}>↑</button><button class="icon-btn" data-action="move-workout-exercise" data-exercise-id="${exercise.id}" data-delta="1" aria-label="Descendre" ${index===workout.exercises.length-1?"disabled":""}>↓</button><button class="icon-btn" data-action="remove-workout-exercise" data-exercise-id="${exercise.id}" aria-label="Retirer" style="color:var(--error)">${icon("delete")}</button></div></div>${exercise.instructionSnapshot ? `<p class="subtitle">${html(exercise.instructionSnapshot)}</p>` : ""}<div class="set-labels set-row"><span></span><small>kg</small><small>reps</small><span></span><span></span></div>${exercise.sets.slice().sort((a,b)=>a.order-b.order).map(set => renderSet(exercise,set)).join("")}${button("Ajouter une série","add-set",{kind:"text",iconName:"add",extra:`data-exercise-id="${exercise.id}"`})}</article>`;
}

function renderSet(exercise,set) {
  const repsNum = Number(set.reps);
  const repsInt = Number.isInteger(repsNum);
  const canMinus = repsInt && repsNum > 1;
  const canPlus = set.reps === "" || (repsInt && repsNum < 999);
  const canValidate = !set.completed && isSetValid(set);
  return `<div class="set-card ${set.completed ? "completed" : ""}" data-set-card="${set.id}"><div class="set-row"><span class="set-number" style="color:${setNumberColor(exercise,set)}">${set.order}</span><input inputmode="decimal" aria-label="Charge série ${set.order}" value="${html(set.weightKg)}" data-set-field="weightKg" data-exercise-id="${exercise.id}" data-set-id="${set.id}"><input inputmode="numeric" aria-label="Répétitions série ${set.order}" value="${html(set.reps)}" data-set-field="reps" data-exercise-id="${exercise.id}" data-set-id="${set.id}"><button class="icon-btn validate-btn ${set.completed ? "done" : ""}" data-action="toggle-set" data-exercise-id="${exercise.id}" data-set-id="${set.id}" aria-label="${set.completed ? "Série validée" : "Valider"}" ${canValidate ? "" : "disabled"}>${set.completed ? "✓" : "○"}</button>${exercise.sets.length > 1 ? `<button class="icon-btn remove-btn" data-action="remove-set" data-exercise-id="${exercise.id}" data-set-id="${set.id}" aria-label="Supprimer la série">${icon("remove")}</button>` : ""}</div><div class="set-tools">${button("−1 rep","adjust-rep",{kind:"text",disabled:!canMinus,extra:`data-exercise-id="${exercise.id}" data-set-id="${set.id}" data-delta="-1"`})}${button("+1 rep","adjust-rep",{kind:"text",disabled:!canPlus,extra:`data-exercise-id="${exercise.id}" data-set-id="${set.id}" data-delta="1"`})}<button type="button" class="rest-link" data-action="edit-set-rest" data-exercise-id="${exercise.id}" data-set-id="${set.id}">${icon("timer")}<span>${set.restBeforeSeconds == null ? "Repos —" : `Repos ${formatRest(set.restBeforeSeconds)}`}</span>${icon("edit")}</button></div><div class="rir-row"><span>RIR</span>${[0,1,2,3].map(v=>`<button type="button" class="rir-chip ${set.rir===v?"active":""}" data-action="set-rir" data-exercise-id="${exercise.id}" data-set-id="${set.id}" data-rir="${v}" aria-label="RIR ${v}">${v}</button>`).join("")}</div></div>`;
}
...
function renderWeight() {
  ...
  return `<section class="screen">${header("Poids","Une mesure quand tu veux, sans jour obligatoire.",`<button class="icon-btn" data-action="export-weight">${icon("download")}</button>`,"🐼")}<section class="card ${appearance.theme === "kawaii" ? "tint-3" : "tint-2"}"><label class="field"><span>Date</span><input id="weight-date" type="date" max="${localDate()}" value="${selectedWeightDate}"></label><label class="field"><span>Poids</span><input id="weight-input" inputmode="decimal" value="${value}" placeholder="0,0"><small>kg</small></label><div class="step-row">${[1,.5,.1].map(v=>button(`+${String(v).replace(".",",")}`,"weight-step",{kind:"ghost",extra:`data-delta="${v}"`})).join("")}</div><div class="step-row">${[-1,-.5,-.1].map(v=>button(`−${String(Math.abs(v)).replace(".",",")}`,"weight-step",{kind:"ghost",extra:`data-delta="${v}"`})).join("")}</div><div class="actions">${entry?button("Supprimer","delete-weight",{kind:"danger",iconName:"delete"}):""}${button(entry?"Mettre à jour":"Enregistrer","save-weight",{iconName:"check"})}</div></section><div class="section-title"><h2>Évolution</h2></div>${rangeChips()}<section class="card">${lineChart(series,{unit:"kg"})}</section>${state.bodyWeights.length?`<div class="list">${state.bodyWeights.slice().sort((a,b)=>b.date.localeCompare(a.date)).slice(0,12).map((item,wi)=>`<button class="list-item ${tinted(wi)}" data-action="select-weight" data-date="${item.date}">${mascotBadge(mascotFor(wi), wi)}<div class="grow"><strong>${number(item.weightKg)} kg</strong><small>${formatDate(item.date)}</small></div>${icon("edit")}</button>`).join("")}</div>`:""}</section>`;
}
...
async function onClick(event){if(event.target.classList?.contains("dialog-backdrop")){modal=null;render();return;}const target=event.target.closest("[data-action]");if(!target)return;const action=target.dataset.action;if(action==="dismiss-dialog"&&event.target.closest("[data-dialog]")&&!event.target.closest("button"))return;try{
  if(action==="tab"){tab=target.dataset.tab;render();scrollTo(0,0)}
   ...
   else if(action==="set-rir"){const rir=Number(target.dataset.rir);mutateSet(target.dataset.exerciseId,target.dataset.setId,set=>({...set,rir:set.rir===rir?null:rir,completed:false}))}
  ...
  else if(action==="save-targets")saveTargetsFromForm()
...
function updateSetInput(input){mutateSet(input.dataset.exerciseId,input.dataset.setId,set=>({...set,[input.dataset.setField]:input.value,completed:false}),false);refreshSetCard(input.dataset.exerciseId,input.dataset.setId)}
function refreshSetCard(exerciseId,setId){
  const draft=activeDraft();
  const exercise=draft?.exercises.find(ex=>ex.id===exerciseId);
  const set=exercise?.sets.find(item=>item.id===setId);
  const card=document.querySelector(`[data-set-card="${CSS.escape(setId)}"]`);
  if(!exercise||!set||!card)return;
  card.classList.toggle("completed",set.completed);
  const btn=card.querySelector(".validate-btn");
  if(btn){
    const ok=!set.completed&&isSetValid(set);
    btn.toggleAttribute("disabled",!ok);
    btn.classList.toggle("done",set.completed);
    btn.setAttribute("aria-label",set.completed?"Série validée":"Valider");
    btn.textContent=set.completed?"✓":"○";
  }
  const num=card.querySelector(".set-number");
  if(num)num.style.color=setNumberColor(exercise,set);
  const repsNum=Number(set.reps),repsInt=Number.isInteger(repsNum);
  card.querySelectorAll('[data-action="adjust-rep"]').forEach(b=>{
    b.toggleAttribute("disabled",Number(b.dataset.delta)<0?!(repsInt&&repsNum>1):!(set.reps===""||(repsInt&&repsNum<999)));
  });
}
...
async function onChange(event){try{if(event.target.dataset.setField&&event.target.tagName==="SELECT")updateSetInput(event.target);if(event.target.id==="weight-date"){selectedWeightDate=event.target.value;render()}if(event.target.id==="nutrition-date"){selectedNutritionDate=event.target.value;editingNutritionId=null;render()}if(event.target.id==="trend-exercise"){document.documentElement.dataset.selectedExercise=event.target.value;render()}if(event.target.id==="restore-file"&&event.target.files[0]){try{const parsed=JSON.parse(await event.target.files[0].text());const {normalizeState}=await import("./state.js");const next=normalizeState(parsed);const summary=backupSummary(next);const detail=`${summary.workouts} séance(s) terminée(s), ${summary.weights} pesée(s), ${summary.nutrition} apport(s)`;const backupEmpty=!summary.workouts&&!summary.weights&&!summary.nutrition;confirmAction("Restaurer cette sauvegarde ?",backupEmpty?`Attention : cette sauvegarde semble vide (${detail}). Toutes les données locales seront quand même remplacées.`:`Contenu : ${detail}. Toutes les données locales seront remplacées.`,()=>{modal=null;persist(next);toast("Sauvegarde restaurée")})}catch(error){toast(`Fichier invalide : ${error.message}`,"error")}}}catch(error){toast(error.message,"error")}}
function onSubmit(event){event.preventDefault();try{if(event.target.id==="nutrition-form"){const data=new FormData(event.target);const next=saveNutrition(state,{id:data.get("entryId")||null,date:selectedNutritionDate,calories:data.get("calories"),protein:data.get("protein")});editingNutritionId=null;persist(next);toast("Apport enregistré")}if(event.target.id==="library-form")saveLibraryForm(event.target)}catch(error){toast(error.message,"error")}}
...
function toggleSet(exerciseId,setId){const draft=activeDraft(),exercise=draft.exercises.find(ex=>ex.id===exerciseId),set=exercise.sets.find(x=>x.id===setId);if(set.completed)return;if(!isSetValid(set))throw new Error("Renseigne la charge et les répétitions");const now=Date.now();const rest=exercise.restStartedAt&&exercise.restTargetSetOrder===set.order?Math.max(0,Math.floor((now-exercise.restStartedAt)/1000)-40):set.restBeforeSeconds;const hasNext=exercise.sets.some(x=>x.order===set.order+1);mutateDraft(d=>({...d,exercises:d.exercises.map(ex=>ex.id===exerciseId?{...ex,sets:ex.sets.map(x=>x.id===setId?{...x,completed:true,restBeforeSeconds:rest}:x),restStartedAt:hasNext?now:null,restTargetSetOrder:hasNext?set.order+1:null}:ex)}));document.activeElement?.blur()}
...
function saveTargetsFromForm(){nutritionTargets=saveNutritionTargets({caloriesKcal:normalizeTargetCalories(document.querySelector("#target-calories").value),proteinGrams:normalizeTargetProtein(document.querySelector("#target-protein").value)});render();toast("Objectifs enregistrés")}
...
function formatRest(totalSeconds){const total=Math.max(0,Math.floor(totalSeconds));return `${Math.floor(total/60)}:${String(total%60).padStart(2,"0")}`}
function setNumberColor(exercise,set){const reps=Number(set.reps);if(set.reps===""||!Number.isInteger(reps))return "var(--muted)";if(reps<exercise.repMinSnapshot)return "var(--warning)";if(reps>exercise.repMaxSnapshot)return "var(--secondary)";return "var(--success)"}
