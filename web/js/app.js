...
function render() {
  ...
  if (draft?.exercises.some(ex => ex.restStartedAt)) timerTick = setInterval(updateTimer, 1000);
}
...
function onInput(event){const input=event.target;if(input.dataset.workoutField)mutateDraft(d=>({...d,[input.dataset.workoutField]:input.value}),false);if(input.dataset.setField&&input.tagName!=="SELECT")updateSetInput(input)}
...
function updateTimer(){const timer=document.querySelector("[data-timer]"),start=Number(timer?.closest("[data-start]")?.dataset.start);if(timer&&start)timer.textContent=elapsed(start)}
