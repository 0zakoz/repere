...
import {
  ...
  nutritionTrend, normalizeTargetCalories, normalizeTargetProtein, nutritionRemaining, normalizeWeight, saveNutrition, saveWeight, sessionStats, shiftDate, skipMissedSlots, startWorkout,
...
function renderScreen() {
  ...
  if (tab === "nutrition") return renderNutritionView({ state, theme: appearance.theme, selectedNutritionDate, editingNutritionId, range });
...
const weightGap = value => `${value > 0 ? "+" : ""}${number(value)}`;
...
function renderWeight() {
  ...
  return `<section class="screen">${header("Poids","Une mesure quand tu veux, sans jour obligatoire.",`<button class="icon-btn" data-action="export-weight">${icon("download")}</button>`,"🐼",appearance.theme)}<section class="card ${appearance.theme === "kawaii" ? "tint-3" : "tint-2"}"><label class="field"><span>Date</span><input id="weight-date" type="date" max="${localDate()}" value="${selectedWeightDate}"></label><label class="field"><span>Poids</span><input id="weight-input" inputmode="decimal" value="${value}" placeholder="0,0"><small>kg</small></label><div class="step-row">${[1,.5,.1].map(v=>button(`+${String(v).replace(".",",")}`,"weight-step",{kind:"ghost",extra:`data-delta="${v}"`})).join("")}</div><div class="step-row">${[-1,-.5,-.1].map(v=>button(`−${String(Math.abs(v)).replace(".",",")}`,"weight-step",{kind:"ghost",extra:`data-delta="${v}"`})).join("")}</div><div class="actions">${entry?button("Supprimer","delete-weight",{kind:"danger",iconName:"delete"}):""}${button(entry?"Mettre à jour":"Enregistrer","save-weight",{iconName:"check"})}</div><label class="field"><span>Objectif</span><input id="weight-goal" inputmode="decimal" value="${state.weightGoalKg ?? ""}" placeholder="ex. 75,5"><small>kg — vide pour retirer</small></label><div class="actions">${button("Définir","save-weight-goal",{iconName:"check"})}</div></section><div class="section-title"><h2>Évolution</h2></div>${rangeChips(range)}<section class="card">${lineChart(series,{unit:"kg",goal:state.weightGoalKg})}</section>${state.weightGoalKg!=null&&trend.length?`<p class="subtitle">Écart avec l'objectif : ${weightGap(trend.at(-1).weightKg-state.weightGoalKg)} kg</p>`:""}${state.bodyWeights.length?`<div class="list">${state.bodyWeights.slice().sort((a,b)=>b.date.localeCompare(a.date)).slice(0,12).map((item,wi)=>`<button class="list-item ${tinted(wi)}" data-action="select-weight" data-date="${item.date}">${mascotBadge(mascotFor(wi), wi)}<div class="grow"><strong>${number(item.weightKg)} kg</strong><small>${formatDate(item.date)}</small></div>${icon("edit")}</button>`).join("")}</div>`:""}</section>`;
}


function renderTrends() {
  const tabs = [["exercises","Exercices"],["sessions","Séances"],["muscles","Muscles"]];
  return `<section class="screen">${header("Tendances","Des données brutes, lisibles et comparables.","","🐱",appearance.theme)}<div class="tabs">${tabs.map(([id,label])=>`<button class="${trendTab===id?"active":""}" data-action="trend-tab" data-tab="${id}">${label}</button>`).join("")}</div>${rangeChips(range)}${trendTab==="sessions"?renderSessionTrends():trendTab==="muscles"?renderMuscleTrends():renderExerciseTrends()}</section>`;
}
...
async function onClick(event){if(event.target.classList?.contains("dialog-backdrop")){modal=null;render();return;}const target=event.target.closest("[data-action]");if(!target)return;const action=target.dataset.action;if(action==="dismiss-dialog"&&event.target.closest("[data-dialog]")&&!event.target.closest("button"))return;try{
  ...
  else if(action==="save-weight-goal")saveWeightGoalFromForm()
...
function saveWeightGoalFromForm(){const raw=document.querySelector("#weight-goal").value.trim();const goal=raw===""?null:normalizeWeight(raw);if(raw!==""&&goal==null)throw new Error("Objectif poids invalide");persist({...state,weightGoalKg:goal});toast("Objectif enregistré")}
function saveTargetsFromForm(){persist({...state,nutritionTargets:{caloriesKcal:normalizeTargetCalories(document.querySelector("#target-calories").value),proteinGrams:normalizeTargetProtein(document.querySelector("#target-protein").value)}});toast("Objectifs enregistrés")}
