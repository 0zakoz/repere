...
export function lineChart(series, { unit = "", target = null, goal = null, empty = "Pas encore de données" } = {}) {
  ...
  const goals = goal != null && Number.isFinite(goal) ? [goal] : [];
  let min = Math.min(...points, ...(target ? [target[0]] : []), ...goals);
  let max = Math.max(...points, ...(target ? [target[1]] : []), ...goals);
  ...
  const goalLine = goals.length ? `<line x1="8" y1="${y(goals[0])}" x2="92" y2="${y(goals[0])}" stroke="var(--secondary)" stroke-width="1" stroke-dasharray="3 2"><title>Objectif : ${goals[0]} ${unit}</title></line>` : "";
  ...
  return `<div class="chart"><div class="chart-axis"><span>${format(max)} ${unit}</span><span>${format((min + max) / 2)} ${unit}</span><span>${format(min)} ${unit}</span></div><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img">${grid}${targetBand}${lines}${goalLine}</svg><div class="chart-dates"><span>${shortDate(dates[0])}</span><span>${dates.length > 1 ? shortDate(dates.at(-1)) : ""}</span></div>${series.length > 1 || goals.length ? `<div class="legend">${series.length > 1 ? series.map((item, i) => `<span><i style="background:${colors[i % colors.length]}"></i>${escape(item.label)}</span>`).join("") : ""}${goals.length ? `<span><i style="background:var(--secondary)"></i>Objectif</span>` : ""}</div>` : ""}</div>`;
}
